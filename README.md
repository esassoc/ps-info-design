...
`theme-ps-info.css` filename itself):

| Placeholder | Meaning | Example |
|---|---|---|
| `Puget Sound Info` | Display name (titles, brand chrome, prose) | `Beacon` |
| `ps-info` | Lowercase id (`data-theme`, theme filename, DocsShell `theme` prop) | `beacon` |
| `ps-info-design` | Repo/dir name + GitHub Pages base | `beacon-design` |
| `ps-info` | npm scope; package name becomes `@ps-info/design`; raw token prefix `--ps-info-*` | `beacon` |
| `P` | Short text mark for the sidebar | `B` |
...
| `Design system and unified-navigation prototypes for Puget Sound Info` | One-line description for the landing hero | `The brand language, patterns, and prototypes for the Beacon platform` |
...
1. Copy this directory to a new sibling of `../ecology`, named `ps-info-design`
...
3. Rename `src/styles/theme-ps-info.css` → `theme-<slug>.css`.
...
├─ astro.config.mjs        base = /ps-info-design/ in prod
├─ tsconfig.json
├─ .gitignore
...
└─ src/
   ...
   ├─ styles/
   │  └─ theme-ps-info.css   the brand theme skeleton
   └─ pages/
