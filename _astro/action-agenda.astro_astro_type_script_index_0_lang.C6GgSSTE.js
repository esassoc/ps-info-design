import{i as l,b as t,a as n}from"./lit-element.DWovE5T-.js";class p extends l{constructor(){super(),this.onInput=e=>{this.value=e.target.value,this.internals.setFormValue(this.value),this.dispatchEvent(new CustomEvent("change",{detail:{value:this.value},bubbles:!0,composed:!0}))},this.label="",this.size="md",this.placeholder="",this.helpText="",this.errorText="",this.required=!1,this.disabled=!1,this.type="text",this.value="",this.prefix="",this.suffix="",this.internals=this.attachInternals()}static{this.formAssociated=!0}static{this.properties={label:{type:String},size:{type:String,reflect:!0},placeholder:{type:String},helpText:{type:String,attribute:"help-text"},errorText:{type:String,attribute:"error-text"},required:{type:Boolean,reflect:!0},disabled:{type:Boolean,reflect:!0},type:{type:String},value:{type:String},prefix:{type:String},suffix:{type:String}}}connectedCallback(){super.connectedCallback(),this.internals.setFormValue(this.value)}render(){const e=!!this.errorText;return t`
  ...
  `}}customElements.get("esa-text-field")||customElements.define("esa-text-field",p);class c extends l{constructor(){super(),this.onDocClick=e=>{this._open&&(e.composedPath().includes(this)||(this._open=!1))},this.onSearchInput=e=>{this._search=e.target.value,this._active=-1,this._open||(this._open=!0)},this.onKeydown=e=>{const r=this.filteredOptions;switch(e.key){case"ArrowDown":if(e.preventDefault(),!this._open)return this.openDropdown();{let i=this._active+1;for(;i<r.length&&r[i].disabled;)i++;i<r.length&&(this._active=i)}break;case"ArrowUp":if(e.preventDefault(),!this._open)return this.openDropdown();{let i=this._active-1;for(;i>=0&&r[i].disabled;)i--;i>=0&&(this._active=i)}break;case"Enter":if(e.preventDefault(),this._open&&this._active>=0){const i=r[this._active];i&&!i.disabled&&this.selectOption(i)}else this._open||this.openDropdown();break;case"Escape":e.preventDefault(),this._open=!1;break;case"Tab":this._open=!1;break}},this.label="",this.options=[],this.size="md",this.placeholder="Select...",this.helpText="",this.errorText="",this.required=!1,this.disabled=!1,this.multiple=!1,this.searchable=!0,this.chipMode=!1,this._search="",this._selected=[],this._open=!1,this._active=-1,this.internals=this.attachInternals()}static{this.formAssociated=!0}static{this.properties={label:{type:String},options:{type:Array},size:{type:String,reflect:!0},placeholder:{type:String},helpText:{type:String,attribute:"help-text"},errorText:{type:String,attribute:"error-text"},required:{type:Boolean},disabled:{type:Boolean,reflect:!0},multiple:{type:Boolean},searchable:{type:Boolean},chipMode:{type:Boolean,attribute:"chip-mode"},_search:{state:!0},_selected:{state:!0},_open:{state:!0},_active:{state:!0}}}connectedCallback(){super.connectedCallback(),document.addEventListener("click",this.onDocClick),this.syncFormValue()}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener("click",this.onDocClick)}set value(e){e==null?this._selected=[]:Array.isArray(e)?this._selected=e:this._selected=[e],this.syncFormValue()}get value(){return this.multiple?this._selected:this._selected[0]??""}get filteredOptions(){const e=this._search.toLowerCase();return e?this.options.filter(r=>r.label.toLowerCase().includes(e)):this.options}get displayValue(){return this._selected.length===0?"":this.options.find(r=>r.value===this._selected[0])?.label??""}get selectedOptions(){return this.options.filter(e=>this._selected.includes(e.value))}get inputValue(){return this.multiple?this._search:this._search||this.displayValue}isSelected(e){return this._selected.includes(e)}syncFormValue(){this.internals.setFormValue(this.multiple?this._selected.join(","):this._selected[0]??null)}emit(){this.syncFormValue(),this.dispatchEvent(new CustomEvent("change",{detail:{value:this.value},bubbles:!0,composed:!0}))}toggleDropdown(){this.disabled||(this._open?this._open=!1:this.openDropdown())}openDropdown(){this.disabled||(this._open=!0,this._active=-1,requestAnimationFrame(()=>{this.renderRoot.querySelector(".input")?.focus()}))}selectOption(e){if(e.disabled)return;const r=e.value;if(this.multiple){const i=this._selected.indexOf(r);this._selected=i>=0?this._selected.filter(o=>o!==r):[...this._selected,r],this._search="",this.emit(),requestAnimationFrame(()=>{this.renderRoot.querySelector(".input")?.focus()})}else this._selected=[r],this._search="",this._open=!1,this.emit()}removeValue(e,r){r?.stopPropagation(),this._selected=this._selected.filter(i=>i!==e),this.emit()}clearSelection(e){e?.stopPropagation(),this._selected=[],this.emit()}renderTags(){const e=this.selectedOptions;if(e.length===0)return null;if(e.length>1)return t`<span class="chip chip--count">
      ...
      <div class="field ${e?"field--error":""}">
        ...
        <div class="container">
          ...
          ${this._open?t`<div class="dropdown" role="listbox">
                ${this.filteredOptions.length===0?t`<div class="option option--empty">No results found</div>`:this.filteredOptions.map((r,i)=>{const o=this.isSelected(r.value);return t`<div
                        class="option ${i===this._active?"option--active":""} ${o?"option--selected":""} ${r.disabled?"option--disabled":""}"
                        role="option"
                        aria-selected=${o}
    ...
    `}chevronIcon(){return t`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
      ...
      stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>`}static{this.styles=n`
    :host {
      display: block;
  ...
  `}}customElements.get("esa-select")||customElements.define("esa-select",c);const s=document.getElementById("aa-vitalsign-select"),a=document.getElementById("aa-vitalsign-options");s&&a?.textContent&&(s.options=JSON.parse(a.textContent));
